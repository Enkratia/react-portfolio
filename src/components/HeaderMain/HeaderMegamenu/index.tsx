...
import { SkeletonHeaderMegamenuList, SkeletonHeaderMegamenuOffer } from "../../Skeletons";
...
export const HeaderMegamenu: React.FC<HeaderMegamenuProps> = ({ index, isMQ1024 }) => {
  const { data, isLoading, isError } = useGetMegamenuLinksQuery();
  ...
  if (isError) {
    console.warn("Failed to load 'megamenu' data.");
  }
              ...
              <ul ref={ulRef} className={s.list}>
                {data[index].nonCategory.map((category, i) => (
                  <li key={i} className={s.item}>
                    <Link
                      to={category.url}
                      className={`${s.link} ${category.red ? s.linkRed : ""}`}>
                      {category.title}
                    </Link>
                  </li>
                ))}
                  ...
                  {data[index].clothes.map((category, i) => (
                    <li key={i} className={s.item}>
                      <Link
                        to={`/${
                          linkNames[index]
                        }/clothes?type%5B0%5D=${category.title.toLowerCase()}`}
                        className={s.link}>
                        {category.title}
                      </Link>
                    </li>
                  ...
                  {data[index].shoes.map((category, i) => (
                    <li key={i} className={s.item}>
                      <Link
                        to={`/${
                          linkNames[index]
                        }/shoes?type%5B0%5D=${category.title.toLowerCase()}`}
                        className={s.link}>
                        {category.title}
                      </Link>
                    </li>
